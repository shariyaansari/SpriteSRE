...
class GitHubClient:
    ...
    def __map_repository(self, data):
        repository = Repository(
            id=data["id"],
            name=data["name"],
            description=data.get("description"),  # * for optional fields, use .get() to avoid KeyError, or a brand new repo may not have a description yet
            owner=data["owner"]["login"],  # !  # Nested GitHub JSON → flat SpriteSRE field
