...
4. Multiple signals are extracted
"""
...
SyntaxError: invalid syntax
    ...
    def test_generic_command_failure(self):
        ...
        assert len(signals) >= 1
        assert signals[0].type == "GENERIC_COMMAND_FAILURE"
...
class TestGeminiAdapterParsing:
  ...
  "confidence": 0.95
    ...
    @pytest.mark.asyncio
    async def test_pipeline_uses_rule_engine_without_llm(self):
        llm = AsyncMock()

        pipeline = DiagnosisPipeline(llm_adapter=llm)

        diagnosis = await pipeline.diagnose(
        ...
        llm.diagnose.assert_not_awaited()
        
    @pytest.mark.asyncio
    async def test_pipeline_falls_back_to_llm(self):
        expected = Diagnosis(
            category="UNKNOWN_FAILURE",
            root_cause="An unknown failure occurred.",
            explanation="The logs do not match a known deterministic rule.",
            suggested_fix="Inspect the workflow logs.",
            confidence=0.8,
        )
